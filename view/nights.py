...
def add_nights_container(page, state):
    ...
    nights_row_min.controls.append(flet.Text("Min", margin=flet.Margin.only(right=4)))
    nights_row_min.controls.append(flet.ElevatedButton("-", width=40, style=flet.ButtonStyle(alignment=flet.Alignment.CENTER), on_click=lambda e:remove_night_from(state, page)))

    _min_nights_text = flet.Text("2")
    nights_row_min.controls.append(_min_nights_text)
    nights_row_min.controls.append(flet.ElevatedButton("+", width=40, on_click=lambda e:add_night_from(state, page)))
    nights_column.controls.append(nights_row_min)
    nights_row_max = flet.Row(expand=True, margin=flet.Margin.only(top=0))
    nights_row_max.controls.append(flet.Text("Max"))
    nights_row_max.controls.append(flet.ElevatedButton("-", width=40, style=flet.ButtonStyle(alignment=flet.Alignment.CENTER), on_click=lambda e:remove_night_to(state, page)))
    _max_nights_text = flet.Text("4")
    nights_row_max.controls.append(_max_nights_text)
    nights_row_max.controls.append(flet.ElevatedButton("+", width=40, on_click=lambda e:add_night_to(state, page)))
